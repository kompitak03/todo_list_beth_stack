...
const app = new Elysia()
  ...
  .post('/clicked', () => <div class="text-blue-600">I'm from the other side</div>)
  .get('/todos', async () => {
...
function TodoItem({ id, content, completed }: Todo) {
  return (
    <div class="flex flex-row space-x-3">
      ...
      <button class="text-red-500" hx-delete={`/todos/${id}`} hx-target="closest div" hx-swap="outerHTML">X</button>
    </div>
  );
}
...
function TodoForm() {
  return (
    <form hx-post="/todos" hx-swap="beforebegin" _="on submit target.reset()" class="flex flex-row space-x-3">
      <input type="text" name="content" id="content" class="border border-black" />
